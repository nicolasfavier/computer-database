...
public class CompanyCli {
	...
	public CompanyCli() {
		client = ClientBuilder.newBuilder().register(JacksonFeature.class)
				.build();
		companyTarget = client
				.target("http://localhost:8080/computer-database-webservice/api/companies");
	...
	public void showCompanies() {
		...
		companies = companyTarget.request(MediaType.APPLICATION_JSON).get(
	 ...
	 * @return int
	 * 
	 *         ask for a company Id, if the id exist in the db return it else
	 *         ask again, if nothing is enter, skip and return null
	 */
